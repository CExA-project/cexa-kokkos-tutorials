...
\begin{frame}[fragile]{Subviews}
  ...
  \begin{minted}{C++}
    auto slicei0 = Kokkos::subview(v, i0, Kokkos::ALL, Kokkos::ALL);
    auto slicei1 = Kokkos::subview(
        v,
        i0, 
        Kokkos::pair(0, v.extent(1)),
        Kokkos::pair(0, v.extent(2))
    );
...
\begin{frame}[fragile]{Subviews}
  ...
  \begin{minted}{C++}
    Kokkos::subview(Kokkos::View<...> view, arg0, ...)
  \end{minted}
  \begin{itemize}
    \item view: First argument to the subview is the view of which a slice will be taken
    \item argN: Slice info for rank N - provide same number of arguments as rank
    \item Options for argN:
      \begin{itemize}
        \item index: integral type single value
        \item partial-range: std::pair or Kokkos::pair of integral types to provide sub-range of a rank’s range [0,N)
        \item full-range: use Kokkos::ALL rather than providing the full range as a pair
      \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Subviews}
  ...
  \begin{itemize}
    ...
    \item Use Kokkos::pair for partial ranges if subview created within a kernel
