...
\begin{frame}{Top500 GPU-CPU system share}
    ...
    \structure{Note:} Does not include exotic accelerators (Intel Knight Landing, etc.)
\end{frame}
    ...
    \begin{columns}
        \begin{column}{0.6\linewidth}
            \begin{itemize}
                ...
                \item Minimum supported version of the compilers detailed \doclink{\href{https://kokkos.org/kokkos-core-wiki/get-started/requirements.html}{in the doc}}
            \end{itemize}
            \begin{minted}{bash}
                cmake \
                    -B ${build_dir} \
            ...
            \begin{minted}[breakafter=/]{cmake}
                ...
                if(NOT Kokkos_FOUND)
                    ...
                    FetchContent_Declare(
                        kokkos
                        URL https://github.com/kokkos/kokkos/releases/download/5.0.0/kokkos-5.0.0.zip
                    )
    ...
    \begin{columns}
        ...
        \begin{column}{0.45\linewidth}
            \begin{itemize}
                ...
                \item \mintinline{c++}{start_index} and \mintinline{c++}{end_index} are the beginning and the end (excluded) of the loop
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \pause
                \item Horrible truth: \mintinline{c++}{result} \highlight{depends} on iteration order!
                \item This is due to rounding errors and unknown iteration order
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item Do not try to read this local result in the kernel (it's a partial result)
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{Function annotation}
  \begin{columns}
    \begin{column}{0.5\linewidth}
      \begin{minted}{C++}
        Kokkos::parallel_for(
    ...
    \begin{column}{0.5\linewidth}
      \begin{itemize}
        \item This code works when compiling with a CPU backend!
        \item Why do we need to use \texttt{KOKKOS\_LAMBDA} inside parallel construct rather than standard C++ lambda?
      \end{itemize}
    \end{column}
  \end{columns}
  \pause
  \pause
  But Fails when compiled on CUDA backend with a cryptic error message:
  \begin{minted}{C++}
  ...
  \begin{columns}
    ...
    \begin{column}{0.55\linewidth}
      \begin{itemize}
        ...
        \item Lambda also need to be annotated
        \item Function be annotated with both \texttt{\_\_host\_\_} and \texttt{\_\_device\_\_} will be callable from the host and the device
        \item \texttt{\_\_host\_\_} is implicit if no annotated is given
...
\begin{frame}[fragile]{Function annotation}
  ...
  \begin{center}
    \begin{tblr}[theme=kokkostable]{colspec=cccc}
      \diagbox{Caller}{Calling}& \texttt{\_\_host\_\_}  & \texttt{\_\_device\_\_} & both\\
...
\begin{frame}[fragile]{Function annotation}
  \begin{columns}
    ...
    \begin{column}{0.55\linewidth}
      \begin{itemize}
        \item \texttt{KOKKOS\_FUNCTION} and \texttt{KOKKOS\_LAMBDA} expand to :
          \begin{itemize}
...
\begin{frame}[fragile]{Function annotation}
  \begin{center}
    \begin{tblr}[theme=kokkostable]{colspec=cccc}
      \diagbox{Caller}{Calling}& No annotation  & \texttt{KOKKOS\_FUNCTION} & \texttt{KOKKOS\_LAMBDA} \\
      No annotation & Ok & Ok & Ok \\
