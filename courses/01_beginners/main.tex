...
\begin{frame}{More about views}
    \begin{itemize}
        \item No hidden cost, allocations only happen at View creation
        \pause
...
\begin{frame}[fragile]{Copy and assignment}
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{minted}{C++}
                Kokkos::View<int*[5]> a("a", 10),
                                      b("b", 15);
                a = b;
                Kokkos::View<int**> c(b);
...
\begin{frame}[fragile]{Extra functionality of \texttt{deep\_copy}}
  \begin{columns}
    \begin{column}{0.55\linewidth}
      \begin{minted}{C++}
        Kokkos::View<float*> view1(N);
        // Fill the view with 5.
        Kokkos::deep_copy(view1, 5.);

        // Create a copy of view1 on the host 
        auto view2 =
          create_mirror_view_and_copy(view1);
      \end{minted}
    \end{column}
    \begin{column}{0.45\linewidth}
      \begin{itemize}
        \item \texttt{deep\_copy} can be used to fill a view with a specific value
        \item \texttt{create\_mirror\_view\_and\_copy} performs both \texttt{create\_mirror\_view} and \texttt{deep\_copy}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item Asynchronism is a complex concept for beginners, it won't be covered in this talk
            \end{itemize}
        \end{column}
    \end{columns}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \pause
                \item Horrible truth: \mintinline{c++}{result} \faIcon{link}\href{https://godbolt.org/z/8z45oxd4W}{depends} on iteration order!
...
\begin{frame}[fragile]{Combining multiple reductions}
    ...
    \structure{Note:} Be careful that the order of the reducers matches the order of the lambda's arguments
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \pause
                \item \texttt{KOKKOS\_FUNCTION} can be used to create functions callable from a parallel kernel
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Conclusion}
    ...
    \begin{itemize}
        ...
        \item How to use tightly nested parallel loops
