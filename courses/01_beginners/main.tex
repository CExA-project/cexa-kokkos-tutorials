    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item This is due to floating point arithmetic being not associative.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item Works like \texttt{parallel\_for} (e.g. specify execution policy etc)
                ...
                \item Update local result in a consistent manner w.r.t. global result.
                \item Only update and not read this local result in the kernel (it's a partial result)
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{About reducers}
    \begin{itemize}
        \item The Reducer object is an instance of a class that implements the Reducer concept
        \item Kokkos provides implementations of the Reducer concept for the most common operations:
        \begin{itemize}
...
\begin{frame}[fragile]{Max reduction example}
    \begin{minted}{C++}
        ...
        Kokkos::parallel_reduce(
            ...
            KOKKOS_LAMBDA (int i, double& local_max_value) {
                ...
                // local operation consistent with global goal
            },
    ...
    \structure{Beware:} \texttt{Kokkos::max} is a mathematical operator, \texttt{Kokkos::Max<double>} defines a Reducer object for obtaining the maximum of a set of double values.
\end{frame}
...
\begin{frame}[fragile]{Min and index reduction example}
    \begin{itemize}
        \item Always think about the type that holds both the intermediate results and the global result
        \item In this case, Kokkos::ValLocScalar<Scalar, Index>: a structure containing val and loc variables
        \item In general, the type can be obtained as $value_type$ of the Reducer object.
    \end{itemize}
    \begin{minted}{C++}
