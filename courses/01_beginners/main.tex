...
\author{Édouard A., Paul G., Matthieu L., Paul Z., and the CExA team}
\institute{CEA}
\date{CEA/Riken Winter School\\15 -- 17\textsuperscript{th} of January 2025\\Barcelona}
\titlegraphic{%
...
\begin{frame}{Schedule of the training}
    \begin{description}[Wednesday 15\textsuperscript{th} 15:30 -- 16:30]
        ...
        \item[Friday 17\textsuperscript{th} 09:00 -- 12:00] Hands-on and introduction to intermediate features
    \end{description}
\end{frame}
    ...
    \begin{columns}
        ...
        \begin{column}{0.5\linewidth}
            \begin{itemize}
                ...
                \item SIMD (Single Instruction Multiple Data) units to vectorise arithmetic operations
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Performance, portability and productivity (aka P3)}
    \begin{columns}[T]
...
\begin{frame}{Going even further}
    \begin{itemize}
        \item Kokkos kernels
        \item Kokkos FFT
        \item Kokkos tools
...
\begin{frame}{Kokkos kernels}
    \begin{itemize}
        \item \githublink{\url{https://github.com/kokkos/kokkos-kernels}}
        \item (Multi)vector dot products, norms, and updates
        \item Sparse matrix-vector multiply and other sparse matrix/dense vector kernels
        \item Sparse matrix-matrix multiply
        \item Graph coloring
        \item Gauss-Seidel with coloring (generalization of red-black)
        \item Other linear algebra and graph operations
    \end{itemize}
\end{frame}

% _____________________________________________________________________________

\begin{frame}[fragile]{Kokkos kernels example: batched LU solver}
    \begin{minted}{C++}
        // LU, jacobian is updated to LU
        KokkosBatched::TeamLU<
            member_type,
            KokkosBatched::Algo::LU::Blocked
        >::invoke(team_member, jacobian, kokkos_TeamLU_tiny);

        // Solve LU x = b, b is updated to x
        KokkosBatched::TeamSolveLU<
            member_type,
            KokkosBatched::Trans::NoTranspose, KokkosBatched::Algo::LU::Blocked
        >::invoke(team_member, jacobian, b);
...
\begin{frame}{Kokkos FFT}
    \begin{itemize}
        \item \githublink{\url{https://github.com/kokkos/kokkos-fft}}
        \item Interfaces between Kokkos and de facto standard FFT libraries
        \begin{itemize}
            \item FFTW
            \item cuFFT
            \item hipFFT/rocFFT
...
\begin{frame}[fragile]{Kokkos FFT example: 1D real to complex transform}
    \begin{minted}{C++}
        View1D<double> x("x", n);
...
\begin{frame}{Kokkos tools}
    \begin{itemize}
        ...
        \item Hooks with popular profilers
        \begin{itemize}
            \item VTune
            \item NVIDIA Nsight Systems (with NVTX)
            \item Timemory
        \end{itemize}
    \end{itemize}
\end{frame}
