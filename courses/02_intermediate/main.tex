...
Francesco Rizzi, Implementing the C++ std algorithms library in 
Kokkos: an overview of the main challenges, API differences and
some implementation details, Kokkos cafe @ CEA, Feb. 2024. 
\end{frame}
...
\begin{frame}[fragile]{Inspired by STL Algorithms in C++}
  \begin{itemize}
    ...
    \item Mostly in \texttt{Kokkos::Experimental} namespace, except for Sort. Also, there is Random
  \end{itemize}
...
\begin{frame}[fragile]{Currently available functions}
  ...
  \begin{tblr}[theme=kokkostable]{ll}
    ...
    \texttt{Sorting and related}    & \texttt{sort, sort\_by\_key, is\_sorted, is\_sorted\_until} \\
    \texttt{Minimum/Maximum}    & \texttt{min\_element, max\_element, minmax\_element} \\
    \texttt{Modifying Sequence}    & \texttt{copy, copy\_if, copy\_n, copy\_backward, move, \\ move\_backward, fill, fill\_n, transform, generate, \\ generate\_n, remove, remove\_if, remove\_copy, \\ remove\_copy\_if, replace, replace\_if, \\ replace\_copy, replace\_copy\_if, swap\_ranges, \\ reverse, reverse\_copy, rotate, rotate\_copy, \\ shift\_left, shift\_right, unique, unique\_copy} \\
    \texttt{Non-modifying Sequence}    & \texttt{adjacent\_find, count, count\_if, equal, all\_of, \\ any\_of, none\_of, find, find\_if, find\_if\_not, \\ find\_end, find\_first\_of, for\_each, for\_each\_n,\\ lexicographical\_compare, mismatch, search, \\ search\_n}
  \end{tblr}
\end{frame}
...
\begin{frame}[fragile]{Currently available functions}
  ...
  \begin{tblr}[theme=kokkostable]{ll}
    Algorithm Family & Functions \\
    \texttt{Numeric}    & \texttt{adjacent\_difference, reduce, exclusive\_scan, \\ inclusive\_scan, transform\_reduce, \\ transform\_exclusive\_scan, transform\_inclusive\_scan} \\
    \texttt{Partitioning}    & \texttt{is\_partitioned, partition\_point, partition\_copy} \\
    \texttt{Random}    & Based on \texttt{Random\_XorShift64\_Pool} \\
    \texttt{Iterators}    & \texttt{begin, cbegin, end, cend, distance, iter\_swap} 
  \end{tblr}
\end{frame}
...
\begin{frame}[fragile]{API usage}
  ...
  \end{itemize}
  \begin{minted}{C++}
    auto ret_val = algo_name(label, space, view(s), extra_args);
    ... 
    auto ret_val = algo_name(label, space, iterator(s), extra_args);
  \end{minted}
\end{frame}

\begin{frame}[fragile]{Example usage}
  \begin{center}
      \includegraphics[width=0.9\textwidth]{replace_algorithm.png}
  \end{center}
\end{frame}

\begin{frame}[fragile]{Example usage}
  \begin{center}
      \includegraphics[width=0.9\textwidth]{min_element_algorithm.png}
  \end{center}
\end{frame}
