...
\begin{frame}[fragile]{What is vectorization}
    \begin{itemize}
        \item To vectorize, we use special instructions called \highlight{intrinsics}
...
\begin{frame}[fragile]{Supported SIMD abis}
    \begin{center}
        \begin{tblr}[theme=kokkostable]{ccc}
        ISA & abis & compatible datatypes \\
        - & \texttt{scalar} & all \\
...
\begin{frame}[fragile]{Initializing a vector}
    \begin{itemize}
        ...
        \item Load the values from memory
            \vspace{-5pt}
            \begin{minted}{C++}
                using namespace Kokkos::Experimental::simd;
...
\begin{frame}[fragile]{Vectorizing a loop}
    ...
    \end{minted}
    \structure{Note:} we iterate by steps of \texttt{VEC\_WIDTH}, since we compute \texttt{VEC\_WIDTH} elements simultaneously.
\end{frame}
...
\begin{frame}[fragile]{Masks}
    \begin{itemize}
        ...
        \item Initialized like a \texttt{simd} vector
            \begin{minted}{C++}
                Kokkos::Experimental::simd_mask<double> m;       // uninitialized
                Kokkos::Experimental::simd_mask<double> m(true); // every value is true
                // true on even indices, false on odd ones
                Kokkos::Experimental::simd_mask<double> m(KOKKOS_LAMBDA(int i) { return i % 2 == 0; });
            \end{minted}
    ...
    \begin{itemize}
        ...
        \item We cannot use an \texttt{if} branch with a vector
            \begin{itemize}
                ...
                \item \structure{Solution:} the \texttt{condition} function
            \end{itemize}
    \end{itemize}
\end{frame}
