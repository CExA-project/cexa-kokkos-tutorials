...
\begin{frame}[fragile]{Currently available functions}
  ...
  \begin{tblr}[theme=kokkostable]{ll}
    ...
    \texttt{Modifying Sequence}    & \texttt{copy, move, fill, transform, remove, replace, \\ reverse, rotate, unique, ...} \\
    \texttt{Non-modifying Sequence}    & \texttt{count, equal, all\_of, any\_of, find, \\ for\_each, search, ...} \\
    \texttt{Numeric}    & \texttt{adjacent\_difference, reduce, exclusive\_scan, \\ inclusive\_scan, ...} \\
    ...
    \texttt{Random}    & \texttt{Random\_XorShift64\_Pool, ...} \\
  \end{tblr}

  \doclink{\url{https://kokkos.org/kokkos-core-wiki/API/algorithms-index.html}}
\end{frame}
...
\begin{frame}[fragile]{Example usage}
  \begin{minted}{C++}
    ...
    int main() {
      ...
      ...
      KE::min_element(def_exec_space, my_view, MyStructLessThanComparator());  
      ...
    }
  \end{minted}
\end{frame}
