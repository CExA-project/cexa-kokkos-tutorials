...
\begin{frame}[fragile]{Acknowledgment}
    ...
    \awesomelink{download}{\href{https://cexa-project.org/news/2024-02-19-fifth-cexa-coffee/kokkos-algorithms.pdf}{Download the presentation}}
\end{frame}
...
\begin{frame}[fragile]{Inspired by STL Algorithms in C++}
    \begin{itemize}
        ...
        \item Needs to include the header \texttt{Kokkos\_StdAlgorithms.hpp}
...
\begin{frame}[fragile]{SIMD Flags}
    \begin{itemize}
        \item On some architectures, SIMD memory operations can be more efficient when the memory is aligned to the size of a vector
        \item All Kokkos SIMD memory operations accept a flag argument
            \begin{itemize}
                \item \texttt{simd\_flag\_default}: unaligned load and stores
                \item \texttt{simd\_flag\_aligned}: aligned load and stores
            \end{itemize}
        \item By default, \texttt{simd\_flag\_default} is used
    \end{itemize}
    \begin{minted}{C++}
        namespace KE = Kokkos::Experimental;
        using simd_type = KE::simd<double>;
        simd_type vec(ptr, KE::simd_flag_aligned);
        vec = KE::simd_unchecked_load<simd_type>(ptr, KE::simd_flag_aligned);
        KE::simd_unchecked_store(vec, ptr, KE::simd_flag_aligned);
    \end{minted}
\end{frame}
...
\begin{frame}{Kokkos tools}
    \begin{itemize}
        ...
        \item Used to have a different version number than Kokkos, version number are in sync since Kokkos 5.2
