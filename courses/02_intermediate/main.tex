...
\begin{frame}[fragile]{Atomics - Race condition}
  But execution between threads is independent. Their is no guarantee over the order of instructions:
    \begin{columns}
        \begin{column}{0.5\linewidth}
            \begin{minted}{C++}
              Kokkos::View<double> res = 0;
        ...
        \end{column}
    \end{columns}
    When several threads are accessing the same data, it may generate \structure{race conditions}.
\end{frame}

\begin{frame}[fragile]{Atomic Operations}
  \texttt{atomic\_inc} execute the \texttt{Write}, \texttt{Read} and \texttt{++} in a single atomic step,
