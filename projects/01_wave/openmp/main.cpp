...
int main(int argc, char* argv[]) {
    ...
    summary << "  - Number of OpenMP threads: " << omp_get_max_threads() << "\n";
    ...
    #pragma omp parallel default(shared) firstprivate(Nx, Ny, dx, dy, A, xp, yp, fwhm, cx2, cy2)
    {
    ...
    for (unsigned int it = 1; it < number_of_iteration ; it++) {
        ...
        #pragma omp parallel default(shared) firstprivate(Nx, Ny, dx, dy, cx2, cy2)
        {
